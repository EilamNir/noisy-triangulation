...
class kalman_filter_from_points_acc:
    ...
    def kalman_step(self, last_X, last_P, current_point, current_cov):
        ...
        P = P - L_k @ self.H @ P
        mahal = np.sqrt(((current_point - self.H @ X_new).T @ np.linalg.inv(
            self.H @ P @ self.H.T + current_cov) @ (current_point - self.H @ X_new)) / len(
            current_point))
        if mahal > 3:
            X_new = X
        ...
        return X_new, P, self.H @ P @ self.H.T, (current_point - self.H @ X_new)
